...
function initAlfredUI() {
  ...
  style.textContent = `
    ...
    #alfred-avatar-wrap {
      ...
      flex-shrink:0; background:#ffffff; border-radius:50%;
    ...
    #alfred-translation {
      font-size:9px; color:rgba(255,255,255,.35); font-style:italic;
      margin-top:4px; width:100%; min-height:14px; text-align:center;
  ...
  setTimeout(async () => {
    setAlfredState('idle');
    const msg = (typeof currentLangue !== 'undefined' && currentLangue === 'nl')
      ? "Goeiedag. Alfred is klaar."
      : "Bonjour. Alfred est en ligne.";
    showBubble(msg);
    if (typeof speak === 'function') await speak(naturaliserTexte(msg), currentLangue);
  }, 700);
}
...
function setAlfredState(state) {
  ...
  switch (state) {
    ...
    case 'think':
      if (body) body.style.animation = 'alfred-sway 1.4s ease-in-out infinite';
...
function startBlinking() {
  function blink() {
    if (curState === 'idle') {
      ...
      const eyeR = document.getElementById('alfred-eye-r');
      if (eyeL && eyeR) {
        eyeL.style.animation = 'alfred-blink .18s ease-in-out';
        eyeR.style.animation = 'alfred-blink .18s ease-in-out .05s';
        setTimeout(() => {
          if (curState === 'idle') { eyeL.style.animation = 'none'; eyeR.style.animation = 'none'; }
        }, 250);
      }
    }
...
function toggleLangue() {
  if (typeof switchLangue === 'function') switchLangue(currentLangue === 'fr' ? 'nl' : 'fr');
  const lbl = document.getElementById('alfred-langue-lbl');
  if (lbl) lbl.textContent = (typeof currentLangue !== 'undefined' && currentLangue === 'nl') ? '🇧🇪 NL' : '🇧🇪 FR';
}
...
function addToHistory(who, text) {
  console.log(`%c[${who.toUpperCase()}]%c ${text.substring(0,100)}`,
    `color:${who==='alfred'?'#14b0bd':'#888'};font-weight:bold`, 'color:inherit');
}
