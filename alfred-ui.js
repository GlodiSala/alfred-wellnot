...
function initAlfredUI() {
  ...
  style.textContent = `
    ...
    #alfred-avatar-wrap {
      ...
      background: #ffffff;
      border-radius: 50%;
      box-shadow: 0 4px 20px rgba(5,69,97,0.3);
    }
    #alfred-dots {
      position: absolute;
      top: 8px;
    ...
    @keyframes alfred-float-z {
      ...
      20%  { opacity: 0.8; }
      100% { opacity: 0; transform: translate(14px,-30px) rotate(14deg); }
    }
  ...
  left.innerHTML = `
    ...
    <div id="alfred-avatar-wrap">
      ...
      <svg id="alfred-svg" viewBox="0 0 346.81 351.58"
        ...
        <!-- CASQUE même couleur que le corps -->
        <path fill="#14b0bd" d="M247.81,79.21c.43-7.09.27-20.44-5.95-27.81-3-3.57-7.26-5.31-13-5.33h-1.52c-10.92,0-109.28,0-109.28,0-6.25,0-10.77,1.83-13.92,5.59-6.32,7.52-6.33,21.23-5.95,27.55h149.62ZM92.6,41.98c6.11-7.28,14.65-10.97,25.37-10.97h110.92c10.32.03,18.57,3.64,24.52,10.72,13.65,16.25,8.82,45.08,8.61,46.3l-1.07,6.25H85.14l-1.1-6.21c-.22-1.19-5.25-29.67,8.56-46.09Z"/>

        <!-- YEUX turquoise — animables -->
        <g id="alfred-eye-l" style="transform-origin:125.46px 62.65px;">
          <circle fill="#14b0bd" cx="125.46" cy="62.65" r="13"/>
        </g>
        <g id="alfred-eye-r" style="transform-origin:221.6px 62.65px;">
          <circle fill="#14b0bd" cx="221.6" cy="62.65" r="13"/>
        </g>

        <!-- BOUCHE turquoise — animable -->
...
function setAlfredState(state) {
  ...
  switch(state) {
    ...
    case 'sleep':
      ...
      if (eyeL) { eyeL.style.transition = 'transform 0.6s ease'; eyeL.style.transform = 'scaleY(0.07)'; }
      if (eyeR) { eyeR.style.transition = 'transform 0.6s ease'; eyeR.style.transform = 'scaleY(0.07)'; }
      if (mouth) mouth.setAttribute('d', MOUTH.flat);
      if (zzz) {
