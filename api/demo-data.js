export default async function handler(req, res) {
  ...
  const KEY = 'alfred_demo_data';

  if (!KV_URL || !KV_TOKEN) {
    return res.status(500).json({ error: 'Base KV non configurée (variables d\'environnement manquantes)' });
  }

  // Contrairement au script (lecture publique), ces données contiennent des
  // numéros de registre national réels — lecture ET écriture protégées.
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const value = JSON.stringify(Object.assign({}, body, { updatedAt }));
