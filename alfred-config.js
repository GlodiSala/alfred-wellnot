...
Jamais : "Excellente question", "Absolument", "Bien sûr", "Certainement", "en tant qu'IA".`,
  ...
  REPLIQUES_FR: [
    ...
    // peut encoder en silence, après la flèche c'est bon"). Un DÉLAI FIXE
    // (DELAI_AVANT_ENCODAGE_CHAMPS_MS, alfred-dom.js — 9s, estimé sur ce que
    // Fariël dit vraiment) laisse ensuite le temps à Fariël de vraiment
    // dicter les 4 infos à voix haute avant que les champs ne s'écrivent —
    ...
    // ORDRE REMIS comme le script officiel (Vendeur/BCE avant Acquéreur/RN)
    // — l'inversion du 03/09 (4e passe) était une mauvaise lecture de la
    // demande ("je parle juste du clic qui associe, pas d'inverser dans le
    // script") : la demande portait sur le clic de RATTACHEMENT DU NOTAIRE
    // (voir PartiesNotaireA plus bas), pas sur l'ordre d'ajout des parties.
    ...
    // ordre ("ça remonte bizarrement", remonté en test live). Libellés FR
    ...
    // Phrase RE-DÉCOUPÉE le 04/09 (3e passe) — demandé explicitement : "en
    // fait ça aurait plus de sens de le dire à la fin de cliquer cette
    // partie... et de le retirer de maxime". La phrase officielle complète
    // ("Je le retrouve dans la base de tous les notaires belges et je le
    // rattache à l'acquéreur. Chaque partie a son notaire.") décrit 3
    // choses à des moments différents : chercher Maxime (ici), le RATTACHER
    // (case "REPRÉSENTE", qui ne se coche en fait que dans la flèche
    // suivante), et confirmer que les 2 parties sont représentées (idem,
    // qu'une fois les 2 cases cochées) — donc "je le rattache..." et
    // "chaque partie a son notaire" ne doivent plus être dits ici, mais
    // dans PartiesNotaireV juste en dessous, qui fait réellement ces 2
    ...
    // Maxime qui vient d'être ajoutée) — PENDANT que la 2e moitié de la
    ...
    { acte: 3, label: 'Sécurité',      emotion: 'assure',      hologrammes: [{ mots: ['rgpd'], titre: 'Conforme RGPD', sous: 'Protection des données', icone: 'cadenas' }, { mots: ['privanot'], titre: 'Évalué par Privanot', sous: 'Sécurité vérifiée', icone: 'badge' }, { mots: ['quittent'], titre: 'Vos données restent en Europe', sous: 'Jamais ailleurs', icone: 'globe' }], texte: "Toutes les données sont hébergées sur des serveurs sécurisés en Europe. Je suis conforme au RGPD, et ma sécurité a été évaluée dans le cadre de Privanot. Vos données ne quittent jamais l'Europe." },
  ...
  REPLIQUES_NL: [
    ...
    { acte: 1, label: 'Disponibilité',  emotion: 'enjoue',      hologrammes: [{ mots: ['24/7'], titre: '24/7, het hele jaar door', sous: 'Nooit gesloten', icone: 'horloge' }], texte: "Ik ben 24/7 beschikbaar, het hele jaar door. Een dossier dat vrijdagavond om 11 uur binnenkomt? U klikt en ik begin meteen." },
    ...
    // bel et bien dans v3_8, différent : "Hij staat in de databank...").
    // Ici la phrase officielle a déjà des points de fin de phrase naturels,
    // rien à réponctuer.
    { acte: 2, label: 'PartiesNotaireA', texte: "Hij staat in de databank van alle Belgische notarissen.", action: 'CreationParties_NotaireAcquereur' },
    // PartiesNotaireV — voir la note FR équivalente.
    ...
    { acte: 2, label: 'RedactionOK', geste: 'hocher', emotion: 'assure', texte: "Ik ben klaar geboren. Laat maar komen!" },
